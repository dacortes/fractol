...
/*   Updated: 2023/02/20 12:00:27 by dacortes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    render (t_fractol *f, int x, int y)
{
	...
	while (++y < HEIGHT)
	{
		...
		while(++x < WIDTH)
		{
			...
			re = f->var.min_re + (x - WIDTH / 2.0) * (f->var.max_re - f->var.min_re) / WIDTH;
			im = f->var.max_im + (y - HEIGHT / 2.0) * (f->var.min_im - f->var.max_im) / WIDTH;
